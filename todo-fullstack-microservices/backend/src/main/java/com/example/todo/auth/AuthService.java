...
public class AuthService {
  ...
  @Transactional public AuthDtos.AuthResponse refresh(String raw){
    ...
    if(rt.getRevokedAt()!=null || rt.getExpiresAt().isBefore(Instant.now())) throw new BadCredentialsException("Refresh token expired or revoked");
  ...
  private AuthDtos.AuthResponse issue(User u){
    ...
    refreshTokens.save(RefreshToken.builder().token(refresh).user(u).expiresAt(Instant.now().plusSeconds(7*24*3600)).build());
