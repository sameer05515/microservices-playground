...
import { resumeItemStyle } from "../../../common/styles/ResumeComponentsStyle";
...
const ResumePreview = ({ resume: providedResume, onCancel }) => {
  ...
  const optionsMap = {
    ...
    SHOW_ALL_COMPANIES_NORMALIZED_DATA: "Show All Companies Normalized Data",
    SHOW_ALL_COMPANIES_WITH_RAW_PROCESSED_DETAILS:
      "Show all companies with raw processed details",
    SHOW_ALL_COMPANIES_REFERENCES: "Show all companies references",
    SHOW_COMPANIES_WITH_PROJECT_NAMES_AND_TECH_STACKS: "Show Companies With Project Names And Tech Stacks",
    ...
    switch (optionValue) {
      case optionsMap.SHOW_ALL_PROJECTS:
        setLastExecutedFilterLogic(
          `resume.companies.map(company => company.projects).flat()`
        );
        setProcessedData(
          resume.companies.map((company) => company.projects).flat()
        );
      ...
      case optionsMap.SHOW_ALL_COMPANIES:
        ...
        break;
      case optionsMap.SHOW_ALL_COMPANIES_NORMALIZED_DATA:
        setLastExecutedFilterLogic(
          "resume.companies.map(company=>({name: company.name, ...company.processedDetails?.metadata  }))"
        );
        setProcessedData(
          resume.companies.map((company) => ({
            name: company.name,
            ...company.processedDetails?.metadata,
          }))
        );
        break;
      ...
      case optionsMap.SHOW_ALL_COMPANIES_REFERENCES:
        ...
        setProcessedData(
          resume.companies.map((company) => ({
            ...
            overAllTenureWithDate:
              company.processedDetails?.metadata?.overAllTenureWithDate || null,
            overAllTenure:
              company.processedDetails?.metadata?.overAllTenure || null,
            lastDesignation:
              company.processedDetails?.metadata?.lastDesignation || null,
            companyWebsiteURL:
              company.processedDetails?.metadata?.companyWebsiteURL || null,
            employeeCode:
              company.processedDetails?.metadata?.lastEmployeeCode || null,
            reasonForChangeActual:
              company.processedDetails?.metadata?.lastReasonForChange?.actual ||
              null,
            reasonForChangeForHR:
              company.processedDetails?.metadata?.lastReasonForChange?.forHR ||
              null,
            lastCTC: company.processedDetails?.metadata?.lastCTC || null,
            projects: company.processedDetails?.metadata?.projects || null,
            techStack: company.processedDetails?.metadata?.techStack || null,
        ...
        break;
      case optionsMap.SHOW_COMPANIES_WITH_PROJECT_NAMES_AND_TECH_STACKS:
        setLastExecutedFilterLogic(
          `
          resume.companies.map((company) => ({
            name: company.name,
            projects: company.processedDetails?.metadata?.projects || [],
            techStack: company.processedDetails?.metadata?.techStack || [],
          }))
          `
        );
        setProcessedData(
          ...
          resume.companies
            .map((company) =>
              company.projects.map((project) => ({
                ...
                companyName: company.name,
                companyTenure:
                  company.processedDetails?.metadata?.overAllTenureWithDate ||
                  null,
                projectTenure:
                  project.processedDetails?.metadata?.tenure || null,
                techStack:
                  project.processedDetails?.metadata?.techStack || null,
              }))
            )
            .flat()
        );
      ...
      default:
        ...
        setProcessedData({
          ...
          summarizedIntroduction:
            resume.processedDetails?.metadata?.summarizedIntroduction || null,
          expertises: resume.processedDetails?.metadata?.expertises || null,
          companies: resume.companies.map((company) => ({
            name: `${company.name} - ${
              company.processedDetails?.metadata?.overAllTenureWithDate || null
            } - ${company.processedDetails?.metadata?.lastCTC || null}`,
            projects: company.processedDetails?.metadata?.projects || null,
            techStack: company.processedDetails?.metadata?.techStack || null,
  ...
  return (
    <div style={resumeItemStyle}>
      ...
      {lastExecutedfilterLogic && (
        ...
        >
          <TextRenderer
            rawText={lastExecutedfilterLogic}
            showRadioButtons={false}
          />
        </CustomCollapse>
      )}

      {errorMessage && <p style={{ color: COLORS.HOT_PINK }}>{errorMessage}</p>}
      ...
      {processedData != null && (
        <CustomCollapse
          ...
          headerText="Preview metadata object with renderCard"
        >
          {renderCard({ title: "Result", objectToBeRendered: processedData })}
        </CustomCollapse>
      )}
