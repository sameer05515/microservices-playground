...
import { getIconDetails, iconFamily } from "./utils";
...
const IconFamilyV6 = () => {
  ...
  const { selectedIcon, IconComponent, next, prev } = useMemo(() => getIconDetails(iconId), [iconId]);
  ...
  const setSelectedIcon = useCallback(
    (icon) => {
      ...
      navigate({
        pathname: "/testing",
        search: createSearchParamsString({ tester: "IconFamilyV6", iconId: icon.uniqueId }),
      });
    },
    [navigate]
  );
