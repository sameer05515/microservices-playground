...
import { getIconDetailsByParentId, getAllIconParents, filterIconFamilyByParentId, getIcon } from "./utils";
    ...
    <div className="mt-0 flex flex-col items-center text-xs">
      ...
      {iconParents.map(({ id, name }) => (
        <NavLink
          ...
          to={{
            pathname: "/testing",
            search: createSearchParamsString({ tester: "IconFamilyV7", iconParentId: id }),
          }}
...
const IconFamilyV7 = () => {
  ...
  const { iconParents, PopupOpenIcon, PopupCloseIcon } = useMemo(
    () => ({
      ...
      PopupOpenIcon: getIcon("BiMinusBack"),
      PopupCloseIcon: getIcon("BiMinus"),
    }),
    []
  );
  ...
  const setSelectedIcon = useCallback(
    (icon, iconParentId) => {
      ...
      navigate({
        pathname: "/testing",
        search: createSearchParamsString({ tester: "IconFamilyV7", iconParentId, iconId: icon.uniqueId }),
      });
    },
    [navigate]
  );
          ...
          <ul>
            {iconFamily.map((node) => (
              <li key={node.uniqueId} className="py-1">
                <TreeNode
                  node={node}
                  setSelectedIcon={(node) => setSelectedIcon(node, iconParentId)}
