...
public class DuplicateScanService {
    ...
    public String start(String rootPath) {
        ...
        ScanJob job = new ScanJob(jobId, rootPath);
        jobs.put(jobId, job);
        runAsync(job);
        return jobId;
    }
            ...
            try {
                ...
                Files.walkFileTree(root, new SimpleFileVisitor<>() {
                    @Override public FileVisitResult visitFile(Path file, BasicFileAttributes a) {
                        if (j.cancelled || Thread.currentThread().isInterrupted()) return FileVisitResult.TERMINATE;
                        if (a.isRegularFile()) all.add(file);
                ...
                if (j.cancelled) { j.status = "CANCELLED"; j.message = "Scan cancelled"; return; }

                j.status = "HASHING";
                Map<Long,List<Path>> bySize = new HashMap<>();
                for (Path p : all) {
                    try { bySize.computeIfAbsent(Files.size(p), x -> new ArrayList<>()).add(p); }
                    catch (Exception ignored) {}
                }

                Map<String,List<Path>> grouped = new HashMap<>();
                List<Path> candidates = new ArrayList<>();
                bySize.values().forEach(list -> { if (list.size() > 1) candidates.addAll(list); });
                j.filesDiscovered = all.size();

                for (Path p : candidates) {
                    if (j.cancelled || Thread.currentThread().isInterrupted()) {
                        j.status = "CANCELLED"; j.message = "Scan cancelled"; return;
                    }
                    try {
                    ...
                    j.filesProcessed++;
                    j.percent = candidates.isEmpty() ? 100 : (int)((j.filesProcessed * 100.0) / candidates.size());
                }
                ...
                for (Map.Entry<String,List<Path>> e : grouped.entrySet()) {
                    ...
                    for (Path p : e.getValue()) {
                        ...
                        fs.add(new DuplicateFile(p.toString(), p.getFileName().toString(), a.size(),
                                Instant.ofEpochMilli(a.lastModifiedTime().toMillis()).toString()));
                    }
                    result.add(new DuplicateGroup(n++, size,
                            e.getKey().substring(e.getKey().indexOf(':') + 1),
                            size * (e.getValue().size() - 1L), fs));
                }
                ...
                if (!j.cancelled) { j.status = "FAILED"; j.message = e.getMessage(); }
            }
        });
    }
    ...
    private static class ScanJob {
        final String jobId, rootPath;
        volatile String status = "QUEUED", message = "Scan queued";
        volatile int percent = 0;
        volatile long filesDiscovered = 0, filesProcessed = 0;
        volatile List<DuplicateGroup> groups = List.of();
        volatile boolean cancelled = false;
        volatile CompletableFuture<?> future;
        ScanJob(String id, String root) { jobId=id; rootPath=root; }
        ScanStatus snapshot() {
            return new ScanStatus(jobId,status,percent,filesDiscovered,filesProcessed,groups.size(),message,groups);
        }
    }
}
